...
def _request_payload(params: dict[str, str]) -> dict[str, Any]:
    ...
    request = Request(url, headers={"User-Agent": "NextCure-Intelligence-Prototype/0.9.34"})
...
def _sponsor_evidence_read(summary: SponsorEvidenceSummary | None) -> str | None:
    ...
    if summary is None:
        return None
    checked = ", ".join(summary.sponsors_checked[:6])
    if summary.result_items:
        top = summary.result_items[:3]
        pieces = [f"{item.sponsor}: {item.title}" for item in top]
        return (
            "Sponsor evidence check found recent result/safety language from mapped trial sponsors: "
            + "; ".join(pieces)
            + ". Treat these as evidence leads to verify against the sponsor release or conference source before using them as efficacy benchmarks."
        )
    if summary.timing_items:
        top = summary.timing_items[:3]
        pieces = [f"{item.sponsor}: {item.title}" for item in top]
        return (
            "Sponsor evidence check found possible data-timing or conference signals from mapped trial sponsors: "
            + "; ".join(pieces)
    ...
    if summary.sponsors_checked:
        return (
            f"Sponsor evidence check did not surface recent ORR/PFS/DOR/safety disclosure language from mapped sponsors checked ({checked}). "
            "Current CDH6 read therefore remains protocol-and-battlefield intelligence, not sponsor-reported efficacy benchmarking."
        )
    return "Sponsor evidence check did not have mapped public-company handles for the surfaced sponsors in this run."
...
def _sponsor_evidence_trend(summary: SponsorEvidenceSummary | None) -> str | None:
    ...
    if summary.result_items:
        ...
        return f"Reported-data watch: mapped sponsor communications surfaced outcome/safety language from {sponsors}; these items should be reconciled against the related trial population, monotherapy/combination setting, and endpoint definitions."
    if summary.timing_items:
        sponsors = ", ".join(dict.fromkeys(item.sponsor for item in summary.timing_items[:4]))
        return f"Upcoming-data watch: mapped sponsor communications surfaced conference or readout timing language from {sponsors}; future runs should watch whether this becomes actual ORR/PFS/DOR/safety evidence."
    return None
...
def build_clinical_trials_intelligence() -> ClinicalTrialsSummary:
    ...
    sponsor_evidence = build_sponsor_evidence_summary(_sponsor_names_for_evidence(records))
